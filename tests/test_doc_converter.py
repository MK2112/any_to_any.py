...
class TestDocumentConverterInit:
    ...
    def test_init_with_custom_locale(self, mock_file_handler, mock_prog_logger, mock_event_logger):
        converter = DocumentConverter(
            file_handler=mock_file_handler,
            prog_logger=mock_prog_logger,
            event_logger=mock_event_logger,
...
class TestDocumentConverterToMarkdown:
    ...
    def test_to_markdown_with_images(
        ...
        mock_image.open.return_value.__enter__.return_value.read.return_value = b"image_data"
...
class TestDocumentConverterToPdf:
    ...
    def test_to_pdf_image_conversion(
        ...
        # Mock file handler
        document_converter.file_handler.join_back.return_value = os.path.join("path", "test_image.jpg")
    ...
    def test_to_pdf_gif_conversion(
        ...
        # Mock file handler
        document_converter.file_handler.join_back.return_value = os.path.join("path", "test_gif.gif")
    ...
    def test_to_pdf_movie_conversion(
        self, mock_join, mock_pixmap, mock_fitz_open, mock_video_clip,
        mock_remove, mock_image_fromarray, document_converter, temp_output_dir
    ):
        # Setup mocks
        mock_doc = MagicMock()
        mock_page = MagicMock()
        ...
        document_converter.file_handler.join_back.return_value = os.path.join("path", "test_movie.mp4")
        mock_join.side_effect = lambda *args: "/".join(args)
...
class TestDocumentConverterToSubtitles:
    ...
    def test_to_subtitles_success(
        self, mock_get_translation, mock_getsize, mock_exists, mock_subprocess,

...
def test_empty_directory(converter_instance, tmp_path):
    empty_dir = tmp_path / "empty"
    empty_dir.mkdir()
    with pytest.raises(SystemExit):
        converter_instance.run([str(empty_dir)], format="mp3", output=str(tmp_path), framerate=None, quality=None, merge=False, concat=False, delete=False, across=False, recursive=False, dropzone=False, language=None)

def test_permission_error_on_output(converter_instance, tmp_path):
...
def test_invalid_quality_value(converter_instance, tmp_path):
...
def test_large_file_conversion(converter_instance, tmp_path):
